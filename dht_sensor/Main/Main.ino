...
Connect both temperature sensors and configure them to measure dry and wet bulb temperatures.
...
// MQTT topics
#define AWS_IOT_PUBLISH_TOPIC   "RANDOM_STUFF/pub"
#define AWS_IOT_SUBSCRIBE_TOPIC "RANDOM_STUFF/sub"
...
void NTPConnect() {
  ...
  now = time(nullptr);
  while (now < 1510592825) {  // Wait until time is synchronized
    delay(500);
    Serial.print(".");
    now = time(nullptr);
  }
...
void messageReceived(char *topic, byte *payload, unsigned int length) {
  ...
  // Example: Parse the incoming message
  ...
  if (!error) {
    if (doc.containsKey("Command")) {
      ...
      if (command == "PUBLISH_NOW") {
        Serial.println("Received command to publish immediately.");
        publishMessage(); // Publish a message immediately on receiving this command
      }
    }
  } else {
